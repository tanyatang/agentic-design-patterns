...
路由将条件逻辑引入Agent的操作框架，使其能够从固定的执行路径转变为这样一种模型：Agent动态评估特定标准，从一组可能的后续操作中进行选择。这使得系统行为更加灵活且具有上下文感知能力。
...
路由模式是构建真正动态响应式Agent系统的关键步骤。通过实现路由，我们超越了简单的线性执行流，使Agent能够智能决策如何处理信息、响应用户输入及利用可用工具或子Agent。
